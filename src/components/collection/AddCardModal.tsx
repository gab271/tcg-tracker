...
export default function AddCardModal({ isOpen, onClose, onAddCard }: AddCardModalProps) {
  ...
  const { data: results = [], isFetching } = useCardSearch(searchTerm);
            ...
            <div className="p-6 pb-4 border-b border-gray-800 flex justify-between items-center bg-vault-800/50">
              <div>
                ...
                <p className="text-xs text-gray-400">
                  Search the global database to add cards to your collection.
                </p>
              </div>
              <button
                onClick={onClose}
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                  {results.map((card) => {
                    ...
                    return (
                      <div
                        key={card.id}
                        className="bg-vault-800 rounded-lg p-3 border border-gray-800 hover:border-gold-500/30 transition-all group flex flex-col"
                      >
